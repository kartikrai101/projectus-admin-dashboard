...
const DeleteModal = ({deleteRoleId, setDeleteRole, setRoleDeleted}) => {
        ...
        <>
            ...
            <div className="min-w-[500px] flex flex-col items-center p-[20px] fixed z-1000 top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-[#FFFFFF] rounded-[10px] shadow-xl">
                ...
                <div className='flex flex-col items-start mt-[20px]'>
                    <p className='text-[26px] text-[#4B465C] text-center font-semibold'>Are you sure you want to delete Role ID {deleteRoleId}?</p>
                    <p className='text-[#4B465C] mt-[20px] text-[14px]'>Role ID {deleteRoleId} will be permanently deleted.</p>
                </div>
                <div className='flex justify-center items-center space-x-3 mt-[35px]'>
                    <button onClick={() => setDeleteRole(false)} className='bg-[#A8AAAE29] text-[#A8AAAE] px-[15px] py-[5px] text-center rounded-[5px] font-medium'>CANCEL</button>
                    <button onClick={() => {
                        setDeleteRole(false)
                        setRoleDeleted(true)
